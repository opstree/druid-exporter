...
func Collector() *MetricCollector{
	return &MetricCollector{
		...
		DruidTasks: prometheus.NewDesc("druid_tasks",
			"Datasources present",
			[]string{"datasource", "index_group_id", "task_status"}, nil,
		),
	}
}
...
func (collector *MetricCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, data := range GetDruidTasks() {
		ch <- prometheus.MustNewConstMetric(collector.DruidTasks, prometheus.GaugeValue, float64(1), fmt.Sprintf("%v",data["dataSource"]), fmt.Sprintf(data["groupId"]), fmt.Sprintf(data["status"]))
	}
}
